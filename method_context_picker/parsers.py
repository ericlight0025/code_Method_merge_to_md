"""Java、JavaScript 與 JSP 的輕量內建解析器。
...
SUPPORTED_SUFFIXES = {".java": "java", ".js": "javascript", ".jsp": "jsp"}
...
def parse_file(file_path: str | Path) -> list[MethodInfo]:
    """解析單一 Java/JavaScript 檔案；JSP 以完整檔案處理，不列 method。"""
...
def find_source_files(directory: str | Path) -> list[Path]:
    """遞迴尋找資料夾內所有支援的 Java / JavaScript / JSP 檔案。"""
...
def parse_source(
    ...
    if resolved_language not in {"java", "javascript", "jsp"}:
        raise ValueError("language 必須是 java、javascript 或 jsp")

    # JSP 可能混合 HTML、JSTL、EL 與 Java scriptlet；第一版不做 method 解析，
    # 交由 GUI 與匯出器以完整檔案保留，避免誤刪模板內容。
    if resolved_language == "jsp":
        return []
